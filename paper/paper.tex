...
Complementary controls (a boundary-balanced test split, matched
Gaussian-noise nulls, and per-image paired bootstrap) bound every source
of optimistic bias we could identify.
...
(regional means $-3$ to $-19\pp$); mid-stack duplication is nearly free;
late-stack duplication is essentially undetectable ($|\Delta|<0.4\pp$),
whereas in LLMs late-layer duplication is catastrophic.
...
\citet{hankins2025sapirwhorf} provided the representational account. Depth
up-scaling~\citep{kim2024solar} exploits related structure with
subsequent fine-tuning; we study the pure inference-time intervention.
...
architecture}: architecture, stimuli, and analysis are held fixed between
the two models. This is consistent with the observation that for a
caption-matching objective, rendering style and background \emph{are}
content---a caption of a white star on purple mentions the
purple---and echoes, at the level of representational geometry, known
appearance biases of contrastively trained
vision models~\citep{geirhos2019imagenet}. Second, \dino{}'s band runs to
...
\begin{figure}[t]
\centering
\includegraphics[width=0.62\linewidth]{figures/depth_profile.pdf}
...
beneficial band & \ding{51} Directionally correct (all regional means
$\le 0$; nothing confirmed), but under-specified: the harm is strongly
depth-structured rather than uniform \\
\addlinespace
...
second-pass effects. Vision transformers may simply sit lower on the same
curve, their single-pass objectives providing little pressure to develop
computations that profit from re-execution.
...
This has a practical corollary: inference-time depth interventions in
vision encoders (adaptive compute, speculative shallow paths, layer
skipping~\citep{fan2020reducing,lad2024remarkable}) can treat the late
stack as a safe region, in sharp contrast to LLM practice.
...
empirical comparison bears this out: on the borderline split, sweep
maxima reach $+21\pp$ (same/different) versus $+4\pp$ on the random
split for the same model and dataset---an order-of-magnitude
demonstration of selection sensitivity
(the borderline-split aggregate map is shown in
Figure~\ref{fig:hardsplit}).
...
\end{subfigure}
\caption{\textbf{Selection sensitivity, demonstrated} (\eva{}, aggregated
raw $\Delta$accuracy). The borderline split (right)---whose test images
were selected by their behavior under the baseline model---paints a
substantially rosier picture than the unbiased random split (left) for
identical interventions. All conclusions in this paper are drawn from the
random split; the pilot design based on \emph{hardest} images (not shown)
was more distorted still.}
\label{fig:hardsplit}
\end{figure}
